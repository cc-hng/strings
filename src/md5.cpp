...
static const WORD32 T[64] = {0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee, 0xf57c0faf,
...
static inline void word32tobytes(const WORD32* input, char* output) {
    int j = 0;
    while (j < 4 * 4) {
        WORD32 v    = *input++;
...
static inline void inic_digest(WORD32* d) {
    d[0] = 0x67452301;
    d[1] = 0xEFCDAB89;
    d[2] = 0x98BADCFE;
    d[3] = 0x10325476;
...
static inline void bytestoword32(WORD32* x, const char* pt) {
    int i;
    for (i = 0; i < 16; i++) {
        int j = i * 4;
        x[i]  = (((WORD32)(unsigned char)pt[j + 3] << 8 | (WORD32)(unsigned char)pt[j + 2]) << 8
...
static inline void put_length(WORD32* x, long len) {
    /* in bits! */
...
std::vector<char> md5(const char* message, size_t len) {
